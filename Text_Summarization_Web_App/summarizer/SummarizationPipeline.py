...
class SummarizationPipeline:
    def __init__(self):
        ...
        self.model = PegasusForConditionalGeneration.from_pretrained("google/pegasus-arxiv")
    ...
    # def generate_summary(self, text, max_tokens=512, batch_size=4):
    #     # Wraping the document in a list for processing
    #     texts = [text]
    ...
    def generate_summary(self, text, max_tokens=512, batch_size=4):
        ...
        with ThreadPoolExecutor() as executor:
            for batch in self._prepare_batches(texts, max_tokens, batch_size):
                futures = [executor.submit(self._summarize_batch, b) for b in batch]
                results = [summary for f in futures for summary in f.result()]  # flatten the list of lists
                return " ".join(results)